    ...
    if (!hasTable) {
      this.schema.withSchema(this.schemaName)
        .createTable(this.tableName, (table) => {
          ...
          table.integer('categoria_id').notNullable().unsigned().references('id').inTable('public.categoria').onDelete('NO ACTION').onUpdate('NO ACTION')
          table.integer('sub_categoria_id').notNullable().unsigned().references('id').inTable('public.sub_categoria').onDelete('NO ACTION').onUpdate('NO ACTION')
          table.integer('anexo_id').nullable().unsigned().references('id').inTable('arquivo.historico_associado').onDelete('SET NULL').onUpdate('SET NULL')
