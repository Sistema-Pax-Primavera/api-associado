...
export default class CreateParcelaValidator {
  ...
  public schema = schema.create({
    ...
    negociacaoId: schema.number.nullableAndOptional(),
    pagamento: schema.array.nullableAndOptional().members(
      schema.number()
    ),
    dataVencimento: schema.date({ format: 'DD/MM/YYYY'}),
    dataPagamento: schema.date.nullableAndOptional({ format: 'DD/MM/YYYY' }),
    valorPagar: schema.number(),
    valorParcela: schema.number(),
    valorAdicional: schema.number(),
    valorAdesao: schema.number(),
    valorPago: schema.number.nullableAndOptional(),
    tipo: schema.enum([
      1, 2, 3
    ])
  })
